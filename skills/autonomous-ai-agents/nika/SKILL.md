...
- `--max-cost-usd` refuses to start (exit 2) when the static floor already
  exceeds the budget; mid-run, the crossing call completes, nothing new
  starts, the run fails `NIKA-1704` (exit 1) with spent-vs-budget
